...
function StudentPerformanceContent() {
  ...
  const fetchData = async (isRefresh = false) => {
    ...
    try {
      const [studentsRes, reportsRes] = await Promise.all([
        HODService.getStudentsList().catch(() => ({ data: { success: false, data: [] } })),
        HODService.getReports().catch(() => ({ data: { success: false, data: [] } }))
      ]);
  ...
  const handleReassign = async (attemptId: string) => {
    ...
    try {
      await HODService.reassignAttempt(attemptId, "HOD reassigned attempt from student analytics portal");
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
        <div className="space-y-1">
          <Button
            variant="ghost"
            size="sm"
            onClick={() => router.push("/hod/analytics")}
            className="text-xs text-muted-foreground hover:text-foreground pl-0 flex items-center gap-1.5 h-7 mb-1"
          >
            <ArrowLeft className="w-4 h-4" />
            <span>Back to HOD Analytics</span>
          </Button>
          <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
            <Users className="w-7 h-7 text-emerald-500 shrink-0" />
            <span>Individual Student Performance Analytics</span>
          </h1>
          <p className="text-xs text-muted-foreground">
            Complete analytical report, exam history, skill breakdown, and proctoring integrity per student profile.
          </p>
        </div>
        ...
        <CardContent className="pt-5 pb-5">
          <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-1">
              ...
              >
                <option value="all">📊 All Students Combined Overview ({students.length})</option>
        ...
        <Card className="bg-gradient-to-r from-primary/10 via-card to-card border-primary/20 shadow-xs">
          <CardContent className="pt-5 pb-5 flex flex-col sm:flex-row items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-bold text-sm text-foreground">Showing Combined Analytics for All Department Students ({students.length})</h3>
