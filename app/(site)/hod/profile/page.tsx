...
export default function HODProfile() {
  ...
  const loadProfile = async () => {
    try {
      const res = await ProfileService.getById(user?.id || 301);
  ...
  const handleSaveProfile = async (e: React.FormEvent) => {
    ...
    if (profile.dob) {
      ...
      if (selectedDob > maxAllowed) {
        toast.error(`Minimum age requirement is 20 years old. DOB must be on or before ${maxAllowed}.`);
        return;
      }
    }
    setSaving(true);
    try {
      await ProfileService.update(user?.id || 301, profile);
    ...
    <div className="space-y-6 animate-in fade-in duration-300">
      <div className="border-b border-border pb-5">
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Manage your personal details, designation details, and security configuration.
        </p>
      </div>
        ...
        <div className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4">
          <div className="flex flex-col items-center text-center space-y-3 pb-4 border-b border-border">
            <div className="w-20 h-20 rounded-full bg-primary/10 flex items-center justify-center text-primary font-bold text-2xl border border-primary/20">
              {profile.name ? profile.name.split(" ").map((n: string) => n[0]).join("").substring(0, 2).toUpperCase() : "H"}
            </div>
            <div>
          ...
          <div className="space-y-3 text-xs text-muted-foreground leading-relaxed">
            ...
            <div className="flex items-center gap-2">
              ...
              <span>{profile.designation || "Head of Department"} • {profile.department || "No department specified"}</span>
            </div>
          </div>
        </div>
            ...
            <form onSubmit={handleSaveProfile} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input 
                    value={profile.designation || "Head of Department (HOD)"} 
                    disabled 
                    readOnly 
                ...
                <div className="space-y-1.5">
                  ...
                  <Input 
                    value={profile.department || "No department assigned"} 
                    disabled 
                    readOnly 
                    className="rounded-xl bg-muted/60 text-muted-foreground cursor-not-allowed font-medium" 
                  />
                ...
                <div className="space-y-1.5">
                  <Label className="flex items-center justify-between">
                    ...
                    <span className="text-[10px] text-muted-foreground font-normal">(20+ Years Minimum)</span>
                  </Label>
                  <Input 
                    type="date" 
                    max={getMaxDobDate()}
                    ...
                    onChange={e => {
                      ...
                      if (val && val > maxAllowed) {
                        toast.error(`Minimum age requirement is 20 years. Please select a date on or before ${maxAllowed}.`);
                        return;
                      }
