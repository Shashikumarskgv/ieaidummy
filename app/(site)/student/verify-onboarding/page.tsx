...
function StudentVerifyOnboardingContent() {
  ...
  const [studentInfo, setStudentInfo] = useState<{ name: string; college: string; email: string } | null>(null);
  ...
  useEffect(() => {
    ...
    async function checkToken() {
      try {
        const response = await StudentService.verifyStudentOnboardingToken(token);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      await StudentService.setupStudentOnboardingPassword(token, password);
      setSuccess(true);
  ...
  if (!valid) {
    return (
      <div className="p-8 text-center space-y-6">
        ...
        <div className="pt-4">
          <Button
            onClick={() => router.replace("/student/auth")}
    ...
    return (
      <div className="p-8 text-center space-y-6 animate-in fade-in zoom-in-95 duration-300">
        ...
        <p className="text-sm text-emerald-600 max-w-sm mx-auto">
          Your student account is now verified and active. You are being redirected to the portal sign-in page...
        </p>
  ...
  return (
    <div className="p-8 space-y-6">
      <div className="text-center">
        ...
        <h2 className="text-2xl font-bold text-foreground">Student Account Setup</h2>
        <p className="text-xs text-muted-foreground mt-1">
          Complete onboarding for Student Portal
        </p>
      </div>

      <div className="bg-muted/40 border border-border/80 rounded-xl p-4 space-y-2 text-sm text-muted-foreground">
        <div>
          <span className="font-semibold text-foreground">Name:</span> {studentInfo?.name}
        </div>
        <div>
          <span className="font-semibold text-foreground">College:</span> {studentInfo?.college}
        </div>
        <div>
          <span className="font-semibold text-foreground">Username / Email:</span> {studentInfo?.email}
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Password Rules */}
        <div className="bg-muted/20 border border-border/60 rounded-xl p-3.5 text-[11px] text-muted-foreground space-y-1.5">
          <span className="font-bold text-foreground block">Password Rules:</span>
          <div className="grid grid-cols-2 gap-x-2 gap-y-1">
            <div className="flex items-center gap-1.5">
              <CheckCircle2 className={`w-3.5 h-3.5 ${password.length >= 6 ? "text-emerald-500" : "text-muted-foreground"}`} />
              <span>Min 6 characters</span>
            </div>
            <div className="flex items-center gap-1.5">
              <CheckCircle2 className={`w-3.5 h-3.5 ${/[A-Z]/.test(password) ? "text-emerald-500" : "text-muted-foreground"}`} />
              <span>One uppercase letter</span>
            </div>
            <div className="flex items-center gap-1.5">
              <CheckCircle2 className={`w-3.5 h-3.5 ${/[0-9]/.test(password) ? "text-emerald-500" : "text-muted-foreground"}`} />
              <span>One number</span>
            </div>
            <div className="flex items-center gap-1.5">
              <CheckCircle2 className={`w-3.5 h-3.5 ${/[^A-Za-z0-9]/.test(password) ? "text-emerald-500" : "text-muted-foreground"}`} />
...
export default function StudentVerifyOnboardingPage() {
  return (
    <div className="min-h-screen w-full flex items-center justify-center bg-background text-foreground font-sans relative px-4">
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-xl overflow-hidden relative z-10 transition-all duration-300">
        ...
        >
          <StudentVerifyOnboardingContent />
        </Suspense>
