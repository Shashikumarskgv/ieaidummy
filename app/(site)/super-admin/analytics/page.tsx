...
export default function SuperAdminAnalyticsPage() {
  ...
  const fetchAnalytics = async (isManualRefresh = false) => {
    ...
    try {
      ...
      if (isManualRefresh) toast.success("Super Admin analytics updated.");
    } catch (err: any) {
      console.error(err);
  ...
  const handleReassign = async (attemptId: string) => {
    ...
    try {
      await SuperAdminService.reassignAttempt(attemptId, "Super Admin reassigned attempt from analytics portal");
  ...
  const placement = examData.placementAnalytics || {};
  const pSummary = placement.summary || {};

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-20 text-muted-foreground min-h-[400px]">
        <div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin mb-4" />
        <span className="text-sm font-semibold">Generating Consolidated College Analytics...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
            ...
            <span>Consolidated College Analytics</span>
          </h1>
          <p className="text-xs text-muted-foreground mt-1">
            Overview of department student reports, exam submissions, passing rates, proctoring audit logs, and job recruitment analytics.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button
            onClick={() => router.push("/super-admin/analytics/student-performance")}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-card border-border shadow-xs hover:shadow-sm transition-shadow">
          <CardContent className="pt-5 space-y-1">
            ...
            <span className="text-[10px] text-muted-foreground block font-medium">Mean score across completed submissions</span>
          </CardContent>
        </Card>
      ...
      <Tabs defaultValue="reports" className="space-y-4">
        ...
        {/* Tab 4: Placement & Job Recruitment Analytics */}
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Card className="bg-card border-border shadow-xs">
              <CardContent className="pt-5 space-y-1">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-2xl font-black text-foreground">{(placement.byCompany || []).length} Companies</span>
            ...
            <Card className="bg-card border-border shadow-xs">
              ...
              <CardContent className="pt-4 space-y-4">
                {(placement.byDepartment || []).length === 0 ? (
                  ...
                  (placement.byDepartment || []).map((d: any, idx: number) => {
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs text-left">
                      ...
                      <tbody className="divide-y divide-border">
                        {(placement.byCompany || []).map((partner: any, idx: number) => (
