...
function StudentPerformanceContent() {
  ...
  const fetchData = async (isRefresh = false) => {
    ...
    try {
      const [studentsRes, reportsRes] = await Promise.all([
        SuperAdminService.getStudentsList().catch(() => ({ data: { success: false, data: [] } })),
        SuperAdminService.getReports().catch(() => ({ data: { success: false, data: [] } }))
      ]);
  ...
  const handleReassign = async (attemptId: string) => {
    ...
    try {
      await SuperAdminService.reassignAttempt(attemptId, "Super Admin reassigned attempt from student analytics portal");
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
        <div className="space-y-1">
          ...
          >
            ...
            <span>Back to Super Admin Analytics</span>
          </Button>
