...
export default function TPOAnalyticsPage() {
  ...
  // Exam analytics state (college-wide)
  ...
  // Placement analytics state (dynamic from DB)
  ...
  const fetchAnalytics = async (isManualRefresh = false) => {
    ...
    try {
      const [examRes, placementRes] = await Promise.all([
        TPOService.getAnalytics().catch(() => ({ data: { success: false, data: null } })),
        TPOService.getDashboardAnalytics().catch(() => ({ data: { success: false, data: null } }))
      ]);

      if (examRes.data?.success && examRes.data?.data) {
        setExamData(examRes.data.data);
      }
      if (placementRes.data?.success && placementRes.data?.data) {
  ...
  const handleReassign = async (attemptId: string) => {
    ...
    try {
      await TPOService.reassignAttempt(attemptId, "TPO reassigned attempt from analytics portal");
  ...
  const filteredAttempts = (examData.attempts || []).filter((att: any) => {
  ...
  const examPerformance = examData.examPerformance || [];
  const activityLogs = examData.activityLogs || [];
  const pSummary = placementData.summary || {};

  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center p-20 text-muted-foreground min-h-[400px]">
        <div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin mb-4" />
        <span className="text-sm font-semibold">Generating College-Wide Student Exam & Placement Analytics...</span>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border-b border-border pb-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-foreground flex items-center gap-2">
            ...
            <span>College Exam & Recruitment Analytics</span>
          </h1>
          <p className="text-xs text-muted-foreground mt-1">
            College-wide student exam performance reports, attempt activity logs, passing ratios, proctoring diagnostics, and job placement analytics.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <Button
            onClick={() => router.push("/tpo/analytics/student-performance")}
      ...
      {/* KPI Stats Summary Cards for Exam & Proctoring */}
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="bg-card border-border shadow-xs hover:shadow-sm transition-shadow">
          <CardContent className="pt-5 space-y-1">
            ...
            <div className="flex items-center justify-between">
              <span className="text-2xl font-black tracking-tight text-foreground">{examSummary.avgScore || 0}%</span>
              <div className="w-8 h-8 rounded-xl bg-blue-500/10 flex items-center justify-center">
                <Award className="w-4 h-4 text-blue-600" />
              </div>
            </div>
            <span className="text-[10px] text-muted-foreground block font-medium">Mean percentage across all college attempts</span>
          </CardContent>
        </Card>
        ...
        <Card className="bg-card border-border shadow-xs hover:shadow-sm transition-shadow">
          <CardContent className="pt-5 space-y-1">
            ...
            <div className="flex items-center justify-between">
              <span className="text-2xl font-black tracking-tight text-emerald-600">{examSummary.passingRate || 0}%</span>
        ...
        <Card className="bg-card border-border shadow-xs hover:shadow-sm transition-shadow">
          <CardContent className="pt-5 space-y-1">
            ...
            <div className="flex items-center justify-between">
              <span className="text-2xl font-black tracking-tight text-amber-600">{examSummary.flaggedCount || 0} Flagged</span>
        ...
        <Card className="bg-card border-border shadow-xs hover:shadow-sm transition-shadow">
          <CardContent className="pt-5 space-y-1">
            ...
            <div className="flex items-center justify-between">
              <span className="text-2xl font-black tracking-tight text-foreground">{examSummary.totalAttempts || 0}</span>
              <div className="w-8 h-8 rounded-xl bg-indigo-500/10 flex items-center justify-center">
                <Users className="w-4 h-4 text-indigo-600" />
              </div>
            </div>
            <span className="text-[10px] text-muted-foreground block font-medium">{examSummary.completedAttempts || 0} completed submissions</span>
          </CardContent>
        </Card>
      </div>

      {/* Main Student Reports & Exam Analytics Tabs */}
      <Tabs defaultValue="reports" className="space-y-4">
        <TabsList className="bg-muted p-1 rounded-xl">
          ...
          <TabsTrigger value="placement" className="rounded-lg text-xs font-semibold">
              ...
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <CardTitle className="text-base font-bold flex items-center gap-2">
                    ...
                    <span>Complete Student Exam Submissions & Reports (All Departments)</span>
                  </CardTitle>
                  <CardDescription className="text-xs mt-0.5">
                    Search and filter individual student test scores, pass/fail status, department details, and proctor integrity.
                  </CardDescription>
                </div>
          ...
          <Card className="bg-card border-border shadow-xs">
            <CardHeader className="pb-3 border-b border-border/60">
              ...
              <CardDescription className="text-xs">
                Aggregated statistics showing pass rates and average scores for every college exam.
              </CardDescription>
            </CardHeader>
              ...
              {examPerformance.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {examPerformance.map((exam: any, idx: number) => (
                    <div 
                      key={idx} 
                      className="p-4 rounded-2xl bg-muted/20 border border-border/70 space-y-3 shadow-2xs hover:border-primary/50 hover:bg-muted/40 transition-all group"
                    >
                      <div className="flex items-center justify-between">
                        <div>
        ...
        {/* Tab 4: Placement & Job Recruitment Analytics (Dynamic from Database) */}
        <TabsContent value="placement" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card className="bg-card border-border shadow-xs">
              <CardContent className="pt-5 space-y-1">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block">Total Published Jobs</span>
                <div className="flex items-center justify-between">
                  <span className="text-2xl font-black text-foreground">{pSummary.publishedJobs || 0}</span>
                  <div className="w-8 h-8 rounded-xl bg-blue-500/10 flex items-center justify-center">
                    <Briefcase className="w-4 h-4 text-blue-600" />
                  </div>
                </div>
                <span className="text-[10px] text-muted-foreground block font-medium">Out of {pSummary.totalJobs || 0} total postings</span>
              </CardContent>
            </Card>

            <Card className="bg-card border-border shadow-xs">
              <CardContent className="pt-5 space-y-1">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block">Job Applications</span>
                <div className="flex items-center justify-between">
                  <span className="text-2xl font-black text-foreground">{pSummary.totalApplications || 0}</span>
                  <div className="w-8 h-8 rounded-xl bg-indigo-500/10 flex items-center justify-center">
                    <Users className="w-4 h-4 text-indigo-600" />
                  </div>
                </div>
                <span className="text-[10px] text-muted-foreground block font-medium">{pSummary.appliedCount || 0} active submissions</span>
              </CardContent>
            </Card>

            <Card className="bg-card border-border shadow-xs">
              <CardContent className="pt-5 space-y-1">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block">Shortlisted Students</span>
                <div className="flex items-center justify-between">
                  <span className="text-2xl font-black text-emerald-600">{pSummary.shortlistedCount || 0}</span>
                  <div className="w-8 h-8 rounded-xl bg-emerald-500/10 flex items-center justify-center">
                    <CheckCircle2 className="w-4 h-4 text-emerald-600" />
                  </div>
                </div>
                <span className="text-[10px] text-emerald-600 font-bold block">Candidates passed screening</span>
              </CardContent>
            </Card>

            <Card className="bg-card border-border shadow-xs">
              <CardContent className="pt-5 space-y-1">
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block">Recruiting Partners</span>
                <div className="flex items-center justify-between">
                  <span className="text-2xl font-black text-foreground">{(placementData.byCompany || []).length} Companies</span>
                  <div className="w-8 h-8 rounded-xl bg-purple-500/10 flex items-center justify-center">
                    <Building className="w-4 h-4 text-purple-600" />
                  </div>
                </div>
                <span className="text-[10px] text-muted-foreground block font-medium">Actively recruiting on portal</span>
              </CardContent>
            </Card>
          </div>

          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Department-wise Job Applications */}
            <Card className="bg-card border-border shadow-xs">
              <CardHeader className="border-b border-border/50 pb-3">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Layers className="w-4 h-4 text-orange-500" />
                  <span>Job Applications by Academic Department</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4 space-y-4">
                {(placementData.byDepartment || []).length === 0 ? (
                  <p className="text-xs text-muted-foreground text-center py-6">No department application data recorded yet.</p>
                ) : (
                  (placementData.byDepartment || []).map((d: any, idx: number) => {
                    const totalApps = pSummary.totalApplications || 1;
                    const pct = Math.round((Number(d.total_applications || 0) / totalApps) * 100);
                    return (
                      <div key={idx} className="space-y-1.5">
                        <div className="flex justify-between items-center text-xs">
                          <span className="font-bold text-foreground truncate max-w-[240px]">{d.department || "General"}</span>
                          <span className="text-[10px] font-bold text-orange-600">{d.total_applications} Applications ({pct}%)</span>
                        </div>
                        ...
                        <div className="flex justify-between text-[10px] text-muted-foreground font-mono">
                          <span>{d.total_students} Active Students</span>
                          <span>{pct}% share</span>
                        </div>
                      </div>
                    );
            ...
            {/* Recruiting Partner Activity */}
            <Card className="bg-card border-border shadow-xs">
              <CardHeader className="border-b border-border/50 pb-3">
                <CardTitle className="text-sm font-bold flex items-center gap-2">
                  <Building className="w-4 h-4 text-blue-500" />
                  <span>Recruiting Partners & Application Distribution</span>
                </CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
                {(placementData.byCompany || []).length === 0 ? (
                  <p className="text-xs text-muted-foreground text-center py-6">No recruiting partners registered yet.</p>
                ) : (
                  <div className="overflow-x-auto">
                    <table className="w-full text-xs text-left">
                      <thead>
                        <tr className="border-b border-border/60 text-muted-foreground pb-2">
                          <th className="py-2">Partner Company</th>
                          <th className="py-2 text-center">Active Jobs</th>
                          <th className="py-2 text-right">Total Applications</th>
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-border">
                        {(placementData.byCompany || []).map((partner: any, idx: number) => (
                          <tr key={idx} className="hover:bg-muted/10">
                            <td className="py-2.5 font-bold text-foreground">{partner.company}</td>
                            <td className="py-2.5 font-mono font-semibold text-center">{partner.total_jobs}</td>
                            <td className="py-2.5 font-mono font-semibold text-right text-emerald-600">{partner.total_applications}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
