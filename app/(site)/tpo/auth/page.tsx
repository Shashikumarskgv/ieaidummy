...
export default function Login() {
  ...
  const handleLogin = async (usr: string, pass: string) => {
    try {
      ...
      const response = await AuthService.login({ username: usr, password: pass, portal: "tpo" }).catch(() => null);
      const user = response?.data?.user || response?.user;

      if (!user || (![2, 4].includes(user.role_id) && user.role !== "TPO")) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      toast.error("Please enter valid username and password (at least 8 characters).");
      return;
    }
  ...
  const handleQuickDemoLogin = async () => {
    setUsername("tpo.corporate@college.edu");
    setPassword("Admin@1234");
    await handleLogin("tpo.corporate@college.edu", "Admin@1234");
  };
    ...
    <div className="min-h-screen w-full flex items-center justify-center bg-background text-foreground font-sans relative px-4">
      ...
      <div className="w-full max-w-md bg-card border border-border rounded-2xl shadow-xl overflow-hidden relative z-10 transition-all duration-300">
        <div className="p-8 pb-4 text-center">
          ...
          <h1 className="text-2xl font-bold tracking-tight text-foreground">TPO Placement Portal</h1>
          <p className="text-xs text-muted-foreground mt-1">Training & Placement Cell • DataQuotes EduTech</p>
        </div>
        ...
        <div className="px-8 mb-2">
          ...
          <div className="bg-primary/5 border border-primary/20 rounded-xl p-3 flex items-center justify-between gap-3 text-xs">
            <div>
              <span className="font-bold text-foreground block">Demo TPO Access</span>
              <span className="text-[11px] text-muted-foreground">Dr. Ananya Sharma (Head - Placements)</span>
            </div>
            <Button
              type="button"
              size="sm"
              ...
              disabled={loading}
              className="bg-primary text-white hover:bg-primary/90 rounded-lg text-xs font-semibold h-8 px-3 shrink-0"
            >
              1-Click Demo
            </Button>
          </div>
