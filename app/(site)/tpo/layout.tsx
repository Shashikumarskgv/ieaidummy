...
export default function TPOLayout({ children }: { children: React.ReactNode }) {
  ...
  const navItems: NavItem[] = [
    { path: "/tpo/dashboard", label: "Dashboard", icon: LayoutDashboard, end: true },
    { path: "/tpo/jobs", label: "Manage Jobs", icon: Briefcase },
    { path: "/tpo/hr-management", label: "HR Management", icon: Users },
    { path: "/tpo/analytics", label: "Analytics", icon: BarChart3 },
    { path: "/tpo/profile", label: "My Profile", icon: User },
  ];

  React.useEffect(() => {
    if (pathname.startsWith("/tpo/auth") || pathname.startsWith("/tpo/verify-onboarding")) {
      setAuthorized(true);
      return;
    }
    let user = AuthService.getCurrentUser();
    const isTPO = user && (user.role_id === 2 || user.role_id === 4 || String(user.role || "").toUpperCase().includes("TPO"));
    if (!user || !isTPO) {
      // Auto-initialize demo TPO session so direct navigation to any TPO route works immediately
      const demoUser = {
        id: 401,
        role_id: 4,
        role: "TPO",
      ...
      StorageService.saveSession("mock_jwt_token_tpo_svce", demoUser);
    }
    setAuthorized(true);
  }, [pathname]);
    ...
    <div className="min-h-screen bg-[#fafafa] text-foreground flex font-sans overflow-x-hidden relative">
      ...
      <header className="lg:hidden flex items-center justify-between w-full h-16 px-4 border-b border-border bg-card absolute top-0 left-0 z-40">
        <div className="flex items-center gap-2">
          <span className="font-bold text-lg text-foreground">TPO Portal</span>
        </div>
        <button
      ...
      >
        <div className="flex flex-col h-full overflow-y-auto scrollbar-hide p-4 space-y-6">
          ...
          <div className="flex items-center justify-between px-2 pt-2">
            <span className="font-bold text-lg tracking-tight text-foreground">TPO Portal</span>
            <button
          ...
          <div className="flex-1 space-y-1">
            {navItems.map((item) => {
              ...
              const isActive = pathname === item.path || (item.path !== "/tpo/dashboard" && pathname.startsWith(item.path));
              return (
                <Link
                  key={item.path}
                  href={item.path}
