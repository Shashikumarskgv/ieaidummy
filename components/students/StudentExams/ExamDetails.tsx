...
const ExamDetails = () => {
  ...
  const handleStartExam = () => {
    ...
    window.open(`/student/StudentExams/examrunner/${examId}`, "_blank", "noopener,noreferrer");
  };

  return (
    <div className="max-w-3xl mx-auto space-y-6 animate-in fade-in duration-300">
      <div className="flex items-center gap-3">
        <Button variant="ghost" size="sm" onClick={() => router.push("/student/StudentExams")}>
      ...
      <div className="bg-card border border-border rounded-2xl p-6 md:p-8 space-y-6 shadow-sm">
        ...
        <div className="flex items-center justify-between border-t border-border/60 pt-6 gap-4">
          <Button variant="outline" onClick={() => router.push("/student/StudentExams")} className="rounded-xl font-bold text-xs">
            Back
          </Button>
          
          {isCompleted ? (
            (exam.end_date && new Date() >= new Date(exam.end_date)) ? (
              <Link href={`/student/StudentExams/reports/${attempt.id}`}>
