...
const LANG_DEFAULTS: Record<string, string> = {
  ...
  r: "# Write your solution\nsolve <- function(input_data) {\n    return(TRUE)\n}\n",
  sql: "-- Write your SQL query solution\nSELECT * FROM table_name;\n",
};
...
export const ExamRunner = () => {
  ...
  useEffect(() => {
    ...
    const initializeAttempt = async () => {
      try {
        ...
        // Bootstrap code editor starter contexts
        const cb: Record<string, any> = {};
        (liveExam.codings || []).forEach((cq: any) => {
          const lang = cq.languages?.[0] || "javascript";
          cb[cq.id] = { lang, code: cq.starter_code || LANG_DEFAULTS[lang] || "" };
        });
        setCodeByQ(cb);
  ...
  const runCode = async (mode: "run" | "submit") => {
    ...
    if (mode === "run") {
      ...
      setTestOutput("");
      try {
        const res = await ExamService.studentRunCode(Number(examId), {
        ...
        if (runResult.error || runResult.status === "server_error" || runResult.success === false) {
          setTestOutput(`Compilation/Runtime Error:\n${runResult.error || runResult.message || "Execution failed"}`);
          toast.error(runResult.error || runResult.message || "Execution failed.");
        } else {
          const passCount = (runResult.details || []).filter((d: any) => d.passed).length;
          const totalCount = (runResult.details || []).length;
          const actualOut = runResult.stdout || runResult.actual_output || (runResult.results && runResult.results[0] ? runResult.results[0].actual_output : "") || "";
          setTestOutput(actualOut ? actualOut : "Execution completed.");
      ...
      try {
        ...
        const resultsList = runResult.results || runResult.details || [];
        const totalCount = resultsList.length;
        const passCount = resultsList.filter((d: any) => d.status === "Passed" || d.passed === true).length;
        const allPassed = runResult.summary?.finalStatus === "Passed" || runResult.passed === true || (totalCount > 0 && passCount === totalCount);
        
        let earnedScore = 0;
        if (runResult.summary?.score !== undefined) {
          earnedScore = (runResult.summary.score / 100) * cq.marks;
        } else if (runResult.total_weight > 0) {
          earnedScore = (runResult.score / runResult.total_weight) * cq.marks;
        }

        const failedIdx = resultsList.findIndex((d: any) => d.status !== "Passed" && !d.passed);
        const failedItem = failedIdx !== -1 ? resultsList[failedIdx] : null;

        setSubmitResult({
          passed: allPassed,
          failedCase: failedIdx !== -1 ? failedIdx + 1 : undefined,
          expected: failedItem ? (failedItem.expected_output !== undefined ? failedItem.expected_output : failedItem.expected) : undefined,
          received: failedItem ? (failedItem.actual_output !== undefined ? failedItem.actual_output : failedItem.actual) : undefined,
          executionTime: runResult.summary?.executionTime !== undefined ? `${runResult.summary.executionTime}ms` : "112ms",
          memory: "23.4 MB",
        ...
        if (allPassed) {
          ...
          toast.error(`Compilation Success: ${passCount}/${totalCount} Test Cases Passed.`);
        }
      } catch (err: any) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 h-full min-h-[460px] animate-in fade-in duration-300 items-stretch">
              ...
              <div className="lg:col-span-7 flex flex-col gap-3 h-full">
                ...
                <div className="flex items-center justify-between gap-2 bg-card border border-border/60 p-2.5 rounded-xl shadow-sm">
                  ...
                  >
                    {((Array.isArray(cq.languages) && cq.languages.length > 0) ? cq.languages : ["python", "javascript", "java", "c", "cpp", "php", "r"]).map((l: string) => (
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button 
                      ...
                      disabled={running || submitting}
                    >
                      {submitting && rightTab === "submit" ? "Submitting..." : "Submit Solution"}
                    </Button>
                  </div>
                </div>
                ...
                <div className="border border-border/80 rounded-2xl overflow-hidden shadow-inner bg-card h-[450px] min-h-[350px] w-full">
                  <Editor
                    ...
                    onChange={(v) => setCodeByQ({ ...codeByQ, [cq.id]: { ...codeByQ[cq.id], code: v || "" } })}
                    theme="vs-dark"
