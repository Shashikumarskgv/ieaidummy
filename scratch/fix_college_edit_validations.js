...
const filePath = path.join(__dirname, '../components/dq-admin/college/edit/[id]/page.tsx');
...
// 2. Replace validateStep, handleNext, handleBack
const oldValStart = '  const validateStep = (stepNum: number) => {';
const oldValEnd = '  const handleSaveChanges = async () => {';

const startIdx = content.indexOf(oldValStart);
const endIdx = content.indexOf(oldValEnd);

if (startIdx === -1 || endIdx === -1) {
  console.error("Could not find validation functions in edit page");
  process.exit(1);
}

const newVal = `  const validateStep = (stepNum: number) => {
    ...
    if (stepNum === 1) {
      ...
      } else if (collegesList.some(c => c.id !== collegeId && c.adminEmail.toLowerCase() === adminEmail.trim().toLowerCase())) {
      ...
      } else if (collegesList.some(c => c.id !== collegeId && c.code.toUpperCase() === collegeCode.trim().toUpperCase())) {
...
content = content.slice(0, startIdx) + newVal + content.slice(endIdx);

// 3. Fix stepper in Edit College Page
const stepperSearchStart = '      {/* Top Padded Card Stepper */}';
const stepperSearchEnd = '      {/* Main Content Area */}';

const stepStartIdx = content.indexOf(stepperSearchStart);
const stepEndIdx = content.indexOf(stepperSearchEnd);

if (stepStartIdx !== -1 && stepEndIdx !== -1) {
...
`;
  content = content.slice(0, stepStartIdx) + cleanStepper + content.slice(stepEndIdx);
}

// 4. Input Replacements
content = content.replace(
...
// Clear errors on dynamic events
content = content.replace(
...
console.log("College Edit Page validations injected!");
